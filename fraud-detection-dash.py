...
class FraudDetectionAPI:
    ...
    def _try_navigator_llm(self, merchant, amount):
        ...
        try:
            ...
            if resp.status_code != 200:
                print(f"Navigator Error: HTTP {resp.status_code}")
                return None
                
            out = resp.json()
            
            if 'choices' not in out or len(out['choices']) == 0:
                print("Navigator Error: No choices in response")
                return None
                
            content = out["choices"][0]["message"]["content"]
            print(f"Navigator raw content: {content}")
            ...
            try:
                ...
                if 'probability' not in data:
                    print("Navigator Error: No probability in JSON")
                    return None
                ...
                if prob < 0.0 or prob > 1.0:
                    print(f"Navigator Error: Invalid probability {prob}")
                    return None
                ...
                if is_suspicious and prob < 0.3:
                    print(f"⚠️ Navigator gave low score ({prob}) for suspicious merchant - overriding")
                    prob = 0.7  # Override to reasonable minimum for suspicious
                
                pred = 1 if prob >= 0.5 else 0
                
                print(f"Navigator Success: prob={prob}, pred={pred}")
                
                return {"success": True, "prediction": pred, "probability": prob}
                
            except (json.JSONDecodeError, ValueError, KeyError) as e:
                print(f"Navigator Parse Error: {e}")
                print(f"Content was: {content[:200]}")
                return None
                
        except Exception as e:
            print(f"Navigator Exception: {e}")
            return None
